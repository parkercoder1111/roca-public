...
import type { ClaudeRunResult } from './tool-executor'
...
const CRM_WEB_BASE = process.env.CRM_WEB_BASE || ''
...
function loadPlaybookSections(): Record<string, string> {
  ...
  for (const filePath of files) {
    ...
    if (keyMatch) {
      ...
      // Fallback: derive key from filename (email-followup.md -> email:followup)
...
/** Convert a playbook key like 'email:followup' to filename. */
...
// TODO: type as CrmEntity | null when CRM entity schemas are defined
async function apiGet(urlPath: string): Promise<any> {
  if (!CRM_API_KEY || !CRM_BASE) return null
  try {
    const resp = await fetch(`${CRM_BASE}/${urlPath}`, {
      headers: CRM_HEADERS,
...
async function fetchMeetingsByEmail(
  ...
  if (!emails.length || !CRM_API_KEY || !CRM_BASE) return []
  ...
  for (const email of emails.slice(0, 2)) {
    try {
      ...
      const url = `${CRM_BASE}/objects/meeting/resources`
      ...
      const json = await resp.json() as { data?: unknown[] }
      for (const r of (json.data ?? []).map(x => x as Record<string, unknown>)) {
        ...
        meetings.push({
          subject: (attrs.title as string | undefined) ?? 'No subject',
          start: (attrs.start as string | undefined) ?? '',
          ai_summary: extractDescription(attrs.summary as CrmField),
          notes: extractDescription(attrs.notes as CrmField),
        })
      }
    } catch (e) {
...
type CrmBlockContent = {
  type?: string
  text?: string | CrmBlockContent[]
  content?: CrmBlockContent[]
  plain_text?: string
  children?: CrmBlockContent[]
  items?: unknown[]
  first_name?: string
  last_name?: string
}
type CrmField = string | number | null | CrmBlockContent | CrmBlockContent[]

/** CRM JSON:API relationship object shape. */
type CrmRelationships = Record<string, { data?: { type: string; id: string } | null }>

/** Minimal shape of a ROCA task object passed through delegation functions. */
interface CrmTask {
  source: string
...
function extractDescription(desc: CrmField): string {
...
function extractBlock(block: CrmBlockContent, lines: string[], depth: number): void {
...
function jsonbFirst(val: CrmField): string | null {
  if (typeof val === 'object' && val !== null && !Array.isArray(val)) {
    const items = val.items
    return Array.isArray(items) && items.length > 0 ? (items[0] as string | null) : null
  }
  return typeof val === 'string' ? val : null
}

/** Format CRM name JSONB. */
function formatName(nameVal: CrmField): string {
  if (typeof nameVal === 'object' && nameVal !== null && !Array.isArray(nameVal)) {
...
function extractEmails(emailVal: CrmField): string[] {
  if (typeof emailVal !== 'object' || emailVal === null || Array.isArray(emailVal)) return []
  const items = emailVal.items
  if (!Array.isArray(items)) return []

  return (items as Array<{ email?: string } | string>)
    .map((item) => {
...
function extractPhones(phoneVal: CrmField): string[] {
  if (typeof phoneVal !== 'object' || phoneVal === null || Array.isArray(phoneVal)) return []
  const items = phoneVal.items
  if (!Array.isArray(items)) return []

  return (items as Array<{ phone_number?: string; number?: string } | string>)
    .map((item) => {
...
interface EnrichmentResult {
  ...
  company: any // TODO: type as CrmEntity | null when CRM entity schemas are defined
  person: any // TODO: type as CrmEntity | null when CRM entity schemas are defined
  deal: any // TODO: type as CrmEntity | null when CRM entity schemas are defined
  meetings: Array<{ subject: string; start: string; ai_summary: string; notes: string }>
  summary: string
}

/** Pull full context from the CRM for this task. */
export async function enrichFromCrm(task: CrmTask): Promise<EnrichmentResult> {
  ...
  // Build transcript excerpts from the CRM API (memsearch excluded in TS port)
  let transcriptExcerpts: Array<{ content: string; heading: string; source: string }> = []

  // Try the CRM API for meeting transcripts
...
function buildSummary(args: BuildSummaryArgs): string {
  ...
  if (CRM_WEB_BASE) parts.push(`CRM: ${CRM_WEB_BASE}/objects/task/records/${taskId}`)
  ...
  if (company) {
    ...
    const status = jsonbFirst(company.status)

    parts.push(`\n### Company: ${name}`)
    if (CRM_WEB_BASE) parts.push(`Link: ${CRM_WEB_BASE}/objects/company/records/${cid}`)
  ...
  if (person) {
    ...
    parts.push(`\n### Person: ${pname}`)
    if (CRM_WEB_BASE) parts.push(`Link: ${CRM_WEB_BASE}/objects/person/records/${pid}`)
  ...
  if (deal) {
    ...
    parts.push(`\n### Deal: ${dname}`)
    if (CRM_WEB_BASE) parts.push(`Link: ${CRM_WEB_BASE}/objects/deal/records/${did}`)
  ...
  if (!person) {
    missing.push('person (no contact linked to this task -- look up in the CRM by company or name from task title)')
  } else if (extractEmails(person.email_addresses).length === 0) {
    missing.push('person email (contact found but no email on record)')
  }
  if (!company) {
    missing.push('company (not linked -- search the CRM if company name appears in task title)')
  }
  if (missing.length > 0) {
...
function buildAnalysisPrompt(task: CrmTask, context: EnrichmentResult, userContext = ''): string {
...
This journal captures patterns from the user's past corrections and preferences.
...
`
  ...
  return `You are the user's CRM analyst.
...
RULES:
- If this involves email: Write the FULL draft (subject line, greeting, body, sign-off). Reference the gmail-drafts skill. NEVER send -- drafts only.
- **EMAIL TEMPLATES**: ALWAYS read \`outputs/templates/email-templates.md\` before writing ANY email. It contains the user's approved templates for pass emails, follow-ups, outreach, meeting requests, etc. Match the tone, structure, and style from the relevant template. If the user asks you to adjust tone/style, check the templates first -- the answer is probably there.
- If this involves CRM: Show exact field values and API endpoints. Base: the CRM API base configured via the CRM_API_BASE environment variable
...
- Use Bash to run the CRM query tool to find contacts (e.g. \`python3 tools/crm/query.py person --filter "name[Contains]=PersonName"\`)
- Use Bash to run the CRM query tool to find company details (e.g. \`python3 tools/crm/query.py company --filter "name[Contains]=CompanyName"\`)
...
CRITICAL: Produce the revised plan AND output in your FIRST response. If you need to look up missing info (email, contact details), use Bash to run the CRM query tool (e.g. \`python3 tools/crm/query.py\`) -- never leave placeholders.`
}

/** Build prompt for actual execution -- Claude gets write tools. */
function buildExecutionPrompt(task: CrmTask, plan: string, context: string): string {
  const rocaPrompt = readRocaFile('roca-prompt.md')

  return `You are the user's CRM analyst. The user reviewed the following plan
...
- For emails: use the gmail-draft script -- DRAFTS ONLY
...
function fallbackPrompt(task: CrmTask, context: EnrichmentResult): string {
  const summary = context.summary
  const titleLower = (task.title ?? '').toLowerCase()

  let action: string
  if (['send', 'email', 'draft', 'follow-up', 'pass'].some(kw => titleLower.includes(kw))) {
    action = 'Draft an email using the gmail-drafts skill (NEVER send -- drafts only)'
    ...
    action = 'Check calendar via the calendar skill and coordinate'
    ...
    action = 'Update CRM records via the CRM REST API'
  ...
  return `### Plan
1. ${action}
2. Review context from the CRM (see below)
...
export async function executePlan(
  ...
  if (!output.trim()) {
    if (result.turns > 1) {
      output = 'Execution completed but no summary was returned. Check the CRM/Gmail for results.'
    } else {
...
Look at the journal's pattern keys (e.g., \`email:followup\`, \`sheets:tracker\`,
...
SECTION 1 (classification key): Just the key, e.g., \`email:followup\`
...
/**
 * Full delegation flow: enrich from the CRM, then run Claude analysis.
