  ...
  start() {
    ...
    this.server = http.createServer((req, res) => {
      ...
      if (url.pathname.startsWith('/webhook/') && req.method === 'POST') {
        ...
        let body = ''
        let bodyBytes = 0
        req.on('data', chunk => { bodyBytes += chunk.length; if (bodyBytes > 1_048_576) { res.writeHead(413); res.end(); req.destroy(); return; } body += chunk })
        req.on('end', async () => {
          try {
    ...
    this.wss.on('connection', (ws) => {
      ...
      ws.on('message', async (raw) => {
        ...
        try {
          ...
          ws.send(JSON.stringify({ id: msg.id, error: e instanceof Error ? e.message : String(e) } satisfies RpcResponse))
        }
      })
