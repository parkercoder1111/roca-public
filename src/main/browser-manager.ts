import { WebContents, webContents, session } from 'electron'
...
function getClaudePath(): string | null {
  ...
  const candidates = [
    ...
    join(process.env.HOME || '', '.nvm/versions/node', 'v24.14.0', 'bin', 'claude'),
  ]
...
const BROWSER_ACTION_SYSTEM = `You are a browser automation agent. You see a screenshot of the current page and execute one action per step.

Actions:
- {"action": "navigate", "url": "<string>"} — go to a URL directly
- {"action": "click", "x": <number>, "y": <number>} — click at pixel coordinates (use center of element)
- {"action": "type", "text": "<string>"} — type text into the currently focused input field
- {"action": "key", "key": "<string>"} — press a key: Enter, Tab, Escape, Backspace, ArrowUp, ArrowDown, ArrowLeft, ArrowRight, Space
- {"action": "scroll", "x": <number>, "y": <number>, "deltaY": <number>} — scroll at position (negative deltaY = down, positive = up)
- {"action": "hover", "x": <number>, "y": <number>} — hover over an element to reveal tooltips, dropdowns, or menus
- {"action": "wait", "ms": <number>} — wait up to 5000ms for page to load or update
- {"action": "done", "summary": "<string>"} — task complete, summarize what was accomplished

Rules:
1. Use "navigate" to go to websites — never click the URL bar
2. Click the CENTER of buttons and links for accuracy
3. For text inputs: click the field first, then use "type"
4. After navigation or clicking links that load new pages, use "wait" to let the page render
5. Be efficient — minimize the number of steps
6. If an element isn't visible, scroll to find it or navigate to the right page

Respond with a brief analysis, then a JSON action in a \`\`\`json code fence.
Only use the Read tool to view the screenshot file. Do not run commands or create files.`

export class BrowserManager {
  private sessions = new Map<number, BrowserSession>()
  /** Reverse map: webContentsId → taskId (for routing popup URLs back to the right task) */
  private wcIdToTaskId = new Map<number, number>()
  ...
  destroy(taskId: number): void {
    ...
    if (session) {
      ...
      // Clean up reverse map entries for this task
      for (const [wcId, tid] of this.wcIdToTaskId) {
        if (tid === taskId) this.wcIdToTaskId.delete(wcId)
      }
  ...
  registerWebContents(taskId: number, wcId: number): void {
    ...
    if (s) {
      ...
      this.wcIdToTaskId.set(wcId, taskId)
      console.log(`[BrowserManager] Registered webContents ${wcId} for task ${taskId}`)
    }
  }

  /** Look up which task owns a given webContents (used for routing popup URLs) */
  getTaskIdForWebContents(wcId: number): number | undefined {
    return this.wcIdToTaskId.get(wcId)
  }
  ...
  /** Public accessor for webview WebContents (used by RPC handlers) */
  getWebContentsPublic(taskId: number): WebContents | null {
    return this.getWebviewContents(taskId)
  }
    ...
    try {
      ...
      if (action.action === 'click') {
        ...
        if (useCdp) {
          ...
          wc.sendInputEvent({ type: 'mouseMove', x: action.x, y: action.y })
        }
      } else if (action.action === 'wait') {
        await new Promise(r => setTimeout(r, Math.min(action.ms || 2000, 5000)))
      }
    } catch (err) {
  ...
  private runClaudeCli(prompt: string, signal: AbortSignal): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      const extraPaths = [
        '/usr/local/bin',
        '/opt/homebrew/bin',
        join(process.env.HOME || '', '.nvm/versions/node/v24.14.0/bin'),
  ...
  async startClaudeLoop(taskId: number, instruction: string): Promise<void> {
    ...
    try {
      ...
      if (msg !== 'Aborted') {
        ...
        session.claudeStatus = `Error: ${msg}`
        this.emitStatus(taskId)
        this.emitThought(taskId, `[error] ${msg}`)
      }
    } finally {
      session.isClaudeActive = false
