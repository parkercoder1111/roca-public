...
import { MobileVoiceMode } from './voice-mode'
...
export function TerminalView({ task, onBack, ptyStatus }: Props) {
  ...
  const [voiceTextToSpeak, setVoiceTextToSpeak] = useState<string | undefined>(undefined)
  const ptyId = `task-${task.id}`

  // Track PTY status transitions for voice: when PTY goes from running → needs_input,
  // Claude finished responding. Grab the last response to speak it.
  const prevPtyStatus = useRef(ptyStatus)
  useEffect(() => {
    if (prevPtyStatus.current === 'running' && ptyStatus === 'needs_input' && termRef.current) {
      // Claude just finished — extract last response from terminal buffer
      const buf = termRef.current.buffer.active
      const lines: string[] = []
      // Grab last 50 lines, look for Claude's response
      const start = Math.max(0, buf.length - 50)
      for (let i = start; i < buf.length; i++) {
        const line = buf.getLine(i)?.translateToString(true) || ''
        lines.push(line)
      }
      // Find the response text (skip tool calls, prompts, status lines)
      const responseText = lines
        .filter(l => l.trim().length > 0)
        .filter(l => !l.startsWith('$') && !l.startsWith('>') && !l.startsWith('─'))
        .filter(l => !l.includes('Tool:') && !l.includes('⏎'))
        .filter(l => l.length > 5)
        .slice(-10)
        .join(' ')
        .trim()
      if (responseText.length > 10) {
        setVoiceTextToSpeak(responseText)
      }
    }
    prevPtyStatus.current = ptyStatus
  }, [ptyStatus])
  ...
  useEffect(() => {
    ...
    // Enable touch scrolling on the xterm viewport (iOS Safari fix)
    setTimeout(() => {
      const viewport = termContainerRef.current?.querySelector('.xterm-viewport') as HTMLElement
      if (viewport) {
    ...
    // Notify PTY backend when terminal dimensions change (e.g. after fit.fit())
  ...
  useEffect(() => {
    const unsubData = on('pty:data', (data: { ptyId: string; data: string }) => {
      if (data.ptyId === ptyId && termRef.current) {
        ...
        // Keep scrollback position in sync so the poll doesn't re-write these bytes
        scrollbackLenRef.current += data.data.length
      }
    })
    ...
    <div className="mv">
      ...
      <div className="mv-header">
        ...
        <div className="mv-header-actions">
          ...
          <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="mv-icon-btn" aria-label="Upload photo">
            {uploading ? (
      ...
      {isConnected && (
        <div style={{
          flexShrink: 0, display: 'flex', gap: 4, padding: '6px 12px',
          background: 'rgba(255,255,255,0.92)', borderTop: '0.33px solid var(--separator)',
          ...
          {[
            ...
            <button key={key.label} onClick={() => api.writePty(ptyId, key.data)}
              style={{
                flexShrink: 0, padding: '5px 10px', borderRadius: 6,
                background: 'rgba(0,0,0,0.05)', border: 'none',
                fontSize: 12, fontWeight: 600, fontFamily: "'SF Mono', Menlo, monospace",
        ...
        {!isConnected && !isStarting ? (
          ...
          <div className="mv-compose">
            <MobileVoiceMode
              ...
              onTextToSpeakConsumed={() => setVoiceTextToSpeak(undefined)}
            />
            <textarea
              ref={inputRef}
              value={input}
              ...
              spellCheck={false}
            />
            <button onClick={sendInput} className="mv-send" disabled={!input.trim()} aria-label="Send">
