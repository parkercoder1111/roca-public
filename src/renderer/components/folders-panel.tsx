...
export function FoldersPanel({
  ...
  return (
    <section>
      {/* Folder section header — mirrors the status-group "chapter" treatment
          so the page reads as a consistent editorial ledger. */}
      <div className="flex items-center gap-2 mb-3 px-1">
        <span className="mono-caps">Folders</span>
        <span className="mono text-[10px] tabular text-text-3/55">{String(folders.length).padStart(2, '0')}</span>
        <span className="flex-1 h-px" style={{ background: 'var(--color-hairline)' }} aria-hidden="true" />
        <button
          onClick={onCreateFolder}
          aria-label="Create new folder"
          title="New folder"
          className="flex items-center gap-1 px-1.5 py-0.5 rounded mono text-[9px] font-medium uppercase tracking-[0.12em] text-text-3 hover:text-purple-1 hover:bg-purple-2 transition-colors cursor-pointer"
        >
...
const FolderItem = React.memo(function FolderItem({
  ...
  folderColors, ptyStatuses = {}, onToggleUrgent, onMergeTasks,
}: FolderItemProps) {
  ...
  return (
    <div
      className="folder-item rounded-lg overflow-hidden transition-all relative"
      data-folder-id={folder.id}
      style={{
        // Flat list treatment — no card border/fill; the folder reads as a
        // section marked only by a thin color rail on the left edge.
        boxShadow: `inset 3px 0 0 0 ${folder.color}`,
      }}
    >
      {/* Folder header row — pl-3 leaves room for the colored inset rail */}
      <div
        className="flex items-center gap-1.5 pl-3 pr-2 py-1.5 cursor-pointer select-none group"
        onClick={() => {
      ...
      >
        ...
        {/* Chevron — small caret in muted ink so it doesn't compete with the rail color */}
        <svg
          className={`w-3 h-3 transition-transform duration-200 shrink-0 text-text-3 ${isCollapsed ? '' : 'rotate-90'}`}
        ...
        {renaming ? (
          ...
          <button
            type="button"
            className="folder-name text-[10.5px] font-semibold uppercase tracking-[0.07em] text-text-2 truncate flex-1 cursor-pointer text-left bg-transparent border-none p-0"
        ...
        {/* Count — mono tabular so lists with many folders read as a tidy column */}
        <span className="mono text-[9.5px] tabular text-text-3/65 shrink-0">{String(tasks.length).padStart(2, '0')}</span>
      ...
      {!isCollapsed && (
        <div className="folder-body" style={{ borderTop: '1px solid var(--color-hairline)' }}>
          <div
            className="folder-drop-zone pl-2 pr-2 py-1.5 space-y-[1px] min-h-[52px]"
          ...
          >
            {tasks.length > 0 ? (
              ...
              <div className="folder-empty-state text-center rounded-md mx-1 my-1 py-3" style={{ border: '1px dashed var(--color-hairline)' }}>
                <p className="mono text-[9px] text-text-3/65 tabular">drop tasks here</p>
              </div>
            )}
          </div>
        </div>
