...
// Base URL for deep-linking a CRM-sourced task back to its source record.
// Set CRM_RECORD_BASE to your CRM's record URL prefix (source_id is appended).
// Empty by default so the "CRM" link is hidden until configured. The renderer
// runs with nodeIntegration off, so `process` may be undefined — read defensively.
const CRM_RECORD_BASE =
  (typeof process !== 'undefined' ? process.env?.CRM_RECORD_BASE : undefined) || ''
...
const COMPLETED_ROW_CLASS = 'flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-surface-1 transition-colors w-full text-left group'

// Status group definitions in display order. Each group owns a dot color that
// both the header and in_progress pulse reuse. `open` has no dot so the eye
// reads it as "default/no-state" — the bulk of the backlog belongs there.
const STATUS_GROUPS: { key: string; label: string; dotColor: string; textColor: string; pulse?: boolean }[] = [
  { key: 'needs_input', label: 'Needs Input', dotColor: 'bg-amber-400', textColor: 'text-amber-500' },
  { key: 'draft_ready', label: 'Draft Ready', dotColor: 'bg-emerald-400', textColor: 'text-emerald-600' },
  ...
  { key: 'waiting', label: 'Waiting', dotColor: 'bg-sky-400', textColor: 'text-sky-600' },
...
const StatusGroup = memo(function StatusGroup({
  group, tasks, selectedTaskId, onSelectTask, onToggleTask, onToggleUrgent, onMergeTasks,
  ...
  return (
    <div className="mb-6">
      {/* Editorial section header — dot + label + count, then a hairline rule
          that runs to the right edge. Reads like a chapter break in a journal. */}
      <h2 className="mb-2.5 px-1 flex items-center gap-2">
        {group.dotColor && (
          <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${group.dotColor} ${group.pulse ? 'animate-pulse' : ''}`} />
        )}
        <span className={`mono-caps ${group.textColor}`}>{group.label}</span>
        <span className="mono text-[10px] tabular text-text-3/55">{String(tasks.length).padStart(2, '0')}</span>
...
export function TaskList({
  openTasks, completedTasks, folders, selectedTaskId, week, isFlowState = false,
  ...
  onReorderFolders, onReorderTasks, onMergeTasks, folderColors, ptyStatuses = {}
}: Props) {
    ...
    <div>
      ...
      >
        {unfolderedTasks.length > 0 ? (
          ...
          <div
            className="task-drop-zone py-10 px-4 rounded-xl hairline text-center"
          ...
          >
            ...
            <div className="text-[12px] font-semibold text-text-1 mb-1">{isFlowState ? 'No active sessions' : 'All clear'}</div>
            <div className="text-[11px] text-text-3/70 max-w-[280px] mx-auto">{
              isFlowState
                ? 'Open a task terminal to start a session — running tasks land here'
                : week === currentIsoWeek()
                  ? 'New tasks, sync results, and triage results land here'
      ...
      {/* Results (completed this week) — section header echoes the "ledger"
          feel with a flanking rule and count in mono tabular. */}
      <section className="mb-8 mt-10">
        <h2 className="mb-3 px-1 flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: 'var(--color-green-bright)' }} aria-hidden="true" />
          <span className="mono-caps" style={{ color: 'var(--color-green-1)' }}>Results</span>
          <span className="mono text-[10px] tabular text-text-3/55">{String(completedTasks.length).padStart(2, '0')}</span>
          <span className="flex-1 h-px" style={{ background: 'var(--color-hairline)' }} aria-hidden="true" />
        </h2>
        {completedTasks.length > 0 ? (
            ...
            {completedTasks.map(task => (
              ...
              >
                <button
                  ...
                  className="w-[16px] h-[16px] rounded-full border-[1.5px] flex-shrink-0 flex items-center justify-center cursor-pointer transition-all border-green-1 bg-green-1 hover:bg-green-1/70 hover:border-green-1/70"
                >
                ...
                {task.source === 'crm' && task.source_id && CRM_RECORD_BASE && (
                  <a
                    href={`${CRM_RECORD_BASE}${task.source_id}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mono text-[9px] tabular px-1.5 py-0.5 rounded-md text-text-3/60 hover:text-text-1 transition-colors"
          ...
          <div className="py-8 px-4 rounded-xl hairline text-center flex flex-col items-center gap-3" style={{ background: 'var(--color-surface-0)' }}>
            ...
            <div>
              <p className="text-[12px] font-semibold text-text-1">{week === currentIsoWeek() ? 'No completions yet' : 'Nothing completed'}</p>
              <p className="text-[11px] text-text-3/70 mt-1">{week === currentIsoWeek() ? 'Completed tasks for this week appear here' : 'No tasks were completed this week'}</p>
            </div>
          </div>
        )}
      </section>
