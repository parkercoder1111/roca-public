...
import type { Upload, AgentRun } from '@shared/types'
...
interface Props {
  ...
  onUploadAdded: () => void
  onClose: () => void
  // Sub-agents the claude session spun up, surfaced above the file list.
  agentRuns?: AgentRun[]
  getAgentEvents?: (runId: string) => Promise<StreamJsonEvent[]>
}
...
// Tile size — large enough for image thumbnails to read as photos, not badges
const TILE_SIZE = 44
...
function FileIcon({ category }: { category: string }) {
  ...
  return (
    <div
      className={`rounded-lg ${config.color} flex items-center justify-center shrink-0 ring-1 ring-inset ring-black/[0.04]`}
      style={{ width: TILE_SIZE, height: TILE_SIZE }}
    >
      <span className="text-[9px] font-bold tracking-wider">{config.ext}</span>
    </div>
  )
}

function ImageThumb({ upload }: { upload: Upload }) {
  ...
  useEffect(() => {
    ...
    setUrl(null)
    setErrored(false)
    window.electronAPI.serveUpload(upload.stored_name).then(data => {
      if (cancelled) return
      if (!data) { setErrored(true); return }
      const blob = new Blob([data], { type: upload.mime_type || 'image/png' })
  ...
  }, [upload.stored_name, upload.mime_type])

  if (errored) return <FileIcon category="image" />

  return (
    <div
      className="rounded-lg overflow-hidden shrink-0 bg-black/[0.04] ring-1 ring-inset ring-black/[0.06]"
      style={{ width: TILE_SIZE, height: TILE_SIZE }}
    >
      {url ? (
        <img
        ...
        <div className="w-full h-full flex items-center justify-center">
          <div className="w-3 h-3 rounded-full bg-purple-1/20 animate-pulse" />
        </div>
      )}
    </div>
  )
...
function FileTile({ upload, category }: { upload: Upload; category: string }) {
  if (category === 'image') return <ImageThumb upload={upload} />
  return <FileIcon category={category} />
}

export function FileSidebar({ taskId, uploads, onUploadAdded, onClose, agentRuns, getAgentEvents }: Props) {
  const [previewUpload, setPreviewUpload] = useState<Upload | null>(null)
  const [previewAgent, setPreviewAgent] = useState<AgentRun | null>(null)
  ...
  isPreviewModeRef.current = previewUpload !== null || previewAgent !== null
  ...
  function handleOpenAgent(run: AgentRun) {
    setPreviewAgent(run)
  ...
  function handleDeleteClick(uploadId: number, e: React.MouseEvent) {
    e.stopPropagation()
    setConfirmDeleteId(uploadId)
  }

  async function handleConfirmDelete(uploadId: number, e: React.MouseEvent) {
    e.stopPropagation()
    try {
  ...
  // Agent preview mode — one sub-agent's live transcript. Re-read the run from
  // the live list by id so its status/steps stay current while open.
  const liveAgent = previewAgent
    ? (agentRuns?.find((r) => r.id === previewAgent.id) ?? previewAgent)
    : null
  if (liveAgent && getAgentEvents) {
    return (
      <div className="relative shrink-0 bg-surface-0 border-l border-black/[0.06] flex flex-col h-full" style={{ width }}>
        ...
        />
        <AgentRunPreview run={liveAgent} getEvents={getAgentEvents} onBack={handleCloseAgent} />
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      {/* Agents — sub-agents the session spun up; red while one is running */}
      {agentRuns && agentRuns.length > 0 && (
        <div className="shrink-0 px-3 pt-2.5 pb-2 border-b border-black/[0.06]">
          <div className="flex items-center gap-2 px-1 pb-1.5">
            <span className="text-[11px] font-semibold text-text-1 tracking-[-0.01em]">Agents</span>
            {agentRuns.some((r) => r.status === 'running') && (
              <span className="flex items-center gap-1 text-[9px] font-semibold text-red-1 bg-red-1/10 px-1.5 py-0.5 rounded-full leading-none">
                <span className="w-1.5 h-1.5 rounded-full bg-red-1 animate-pulse" />
          ...
          <div className="space-y-0.5">
            {agentRuns.map((run) => {
              ...
              return (
                <button
                  key={run.id}
                  onClick={() => handleOpenAgent(run)}
                  className="w-full flex items-center gap-2.5 px-2 py-2 rounded-xl text-left hover:bg-black/[0.03] transition-colors cursor-pointer"
                >
                  <span className={`w-2 h-2 rounded-full shrink-0 ${running ? 'bg-red-1 animate-pulse' : run.status === 'error' ? 'bg-red-1' : 'bg-green-1'}`} />
                  <div className="flex-1 min-w-0">
                    <div className="text-[11px] font-medium text-text-1 truncate leading-tight">{run.title}</div>
                    <div className="text-[9px] text-text-3/70 mt-0.5 truncate">
                      {run.subagentType ? `${run.subagentType} · ` : ''}
                      {running ? 'Running' : run.status === 'error' ? 'Failed' : 'Done'}
                      {run.steps ? ` · ${run.steps} steps` : ''}
                    </div>
                  </div>
                  <svg className="w-3 h-3 text-text-3/40 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              return (
                <div
                  ...
                  className={`rounded-xl transition-colors duration-150 ${
                    isConfirming
                      ? 'bg-red-1/[0.06]'
                      : 'hover:bg-black/[0.03] group-focus-within:bg-black/[0.03]'
                  } group`}
                >
                  {isConfirming ? (
                    <div className="flex items-center gap-2 px-2.5 py-2.5">
                      <FileTile upload={upload} category={cat} />
                      <div className="flex-1 min-w-0">
                        <div className="text-[11px] font-medium text-text-1 truncate leading-tight">{upload.filename}</div>
                        <div className="text-[9px] text-red-1/80 mt-0.5">Delete this file?</div>
                      </div>
                      <button
                        onClick={(e) => handleConfirmDelete(upload.id, e)}
                        className="px-2 py-1 rounded-md text-[9px] font-medium bg-red-1 text-white hover:bg-red-1/90 transition-colors cursor-pointer"
                      >
                        Delete
                      </button>
                      <button
                        onClick={handleCancelDelete}
                        className="px-2 py-1 rounded-md text-[9px] font-medium bg-black/[0.06] text-text-2 hover:bg-black/[0.1] transition-colors cursor-pointer"
                      >
                        Cancel
                      </button>
                    </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <button
                        ...
                        className="p-1 mr-2 rounded-md text-text-3/20 hover:text-red-1 hover:bg-red-1/10 transition-all cursor-pointer"
                        title="Delete file"
                      >
