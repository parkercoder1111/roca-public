...
export function FilePathView() {
  ...
  // ── Load agents (once) ─────────────────────────────────────────────────────
  useEffect(() => {
    api.agentsList().then((list: AgentInfo[]) => setAgents(list)).catch((err: unknown) => console.warn('[FilePathView] initial agentsList failed:', err))
  }, [])

  // ── Poll agent status when context bar is visible ──────────────────────────
  useEffect(() => {
    if (!matchedAgent) return
    const poll = () => api.agentsList().then((list: AgentInfo[]) => {
      setAgents(list)
      const refreshed = list.find(a => a.label === matchedAgent.label)
      if (refreshed) setMatchedAgent(refreshed as AgentInfo)
    }).catch((err: unknown) => console.warn('[FilePathView] poll agentsList failed:', err))
  ...
  useEffect(() => {
    ...
    if (agentDirMatch) {
      // Derive a display label from the agent's directory name
      // (e.g. "data-sync" -> "Data Sync") and match it to a known agent.
      const dirName = agentDirMatch[1]
      const label = dirName.replace(/[-_]+/g, ' ').replace(/\b\w/g, c => c.toUpperCase())
      const match = agents.find(a => a.name.toLowerCase() === label.toLowerCase())
